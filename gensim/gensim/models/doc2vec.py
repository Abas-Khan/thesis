...
'''
from gensim.models.doc2vec_inner import train_document_dbow,train_document_dbow_docs, train_document_dm, train_document_dm_concat
...
# print "In dbow function i have ", vocab_file
    #print model.corpus_count * model.iter/2
    global iterations
    # print "index is ",doctag_indexes
    #print "\r",
    ...
    #for key, value in model.docvecs.doctags.iteritems():
    #    print key, value
    #sys,exit()
    if train_words and learn_words:
        if iterations >= model.corpus_count*5:
            print "back to factory method"
            train_batch_sg_normal(model, [doc_words], alpha, work)
        else:
            print "normal call"
...
def train_document_dm(model, doc_words, doctag_indexes, alpha, work=None, neu1=None,
    ...
    #print "\n the word list  "  
    words = []       
    for i,word in enumerate(word_vocabs):
        words.append(model.wv.index2word[word.index])

    word_list = ' '.join(words)      
    #print "words list is ",word_list,    
    #print "\n"
    for pos, word in enumerate(word_vocabs):
        #print "------main word----------- ",model.wv.index2word[word.index]
        #main_word = model.wv.index2word[word.index]
        #lh, _, rh = word_list.partition(main_word)
        #print "main solution is",
        #print ' '.join(lh.split()[-3:]+[main_word]+rh.split()[:3])
        reduced_window = model.random.randint(model.window)  # `b` in the original doc2vec code
        #print "reduced window size is ",reduced_window
    ...
    #     print "word ",wrd,
        ...
        if model.cbow_mean and count > 1:
            l1 /= count
        neu1e = train_cbow_pair_dis2vec(model, word, word2_indexes, l1, alpha,
...
class Doc2Vec(BaseWordEmbeddingsModel):
    ...
    def __init__(self, documents=None, dm_mean=None, dm=1, dbow_words=0, dm_concat=0, dm_tag_count=1,
                 docvecs=None, docvecs_mapfile=None, comment=None, trim_rule=None, callbacks=(),vocab_file =None ,smoothing=0.75,sampling_param=0.3, objective_param=0.3 ,**kwargs):
        ...
        #print " the vocab file in Doc2Vec is .............. ",self.vocab_file
        ...
        vocabulary_keys = ['max_vocab_size', 'min_count', 'sample', 'sorted_vocab', 'null_word' , 'vocab_file','smoothing']
    ...
    def __init__(self, max_vocab_size=None, min_count=5, sample=1e-3, sorted_vocab=True, null_word=0,vocab_file = None, smoothing = 0.75):
       # print " the vocab_file is  {0}".format(vocab_file)
        super(Doc2VecVocab, self).__init__(
            ...
            sorted_vocab=sorted_vocab, null_word=null_word,vocab_file = vocab_file , smoothing = smoothing)
    def scan_vocab(self, documents, docvecs, progress_per=10000, trim_rule=None):
        logger.info("collecting all words and their counts")
        print"scanning vocabs"
        document_no = -1
        total_words = 0
        min_reduce = 1
