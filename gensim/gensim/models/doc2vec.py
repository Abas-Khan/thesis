...
from gensim.models.word2vec import Word2VecKeyedVectors, Word2VecVocab, Word2VecTrainables, train_cbow_pair,\
    train_sg_pair, train_batch_sg ,train_sg_pair_docs_only
...
try:
    ...
    def train_document_dbow(model, doc_words, doctag_indexes, alpha, work=None,
                            ...
                            word_vectors=None, word_locks=None, doctag_vectors=None, doctag_locks=None , vocab_file = None):
        """
       ...
       # print "In dbow function i have ", vocab_file
        ...
        if train_words and learn_words:
            train_batch_sg(model, [doc_words], alpha, work , vocab_file = vocab_file)
...
class Doc2Vec(BaseWordEmbeddingsModel):
    ...
    def __init__(self, documents=None, dm_mean=None, dm=1, dbow_words=0, dm_concat=0, dm_tag_count=1,
                 docvecs=None, docvecs_mapfile=None, comment=None, trim_rule=None, callbacks=(),vocab_file =None ,smoothing=0.75, **kwargs):
        ...
        for doc in job:
            ...
            if self.sg:
                tally += train_document_dbow(
                    ...
                    doctag_vectors=doctag_vectors, doctag_locks=doctag_locks , vocab_file = self.vocab_file
                )
            elif self.dm_concat:
    ...
    def infer_vector(self, doc_words, alpha=0.1, min_alpha=0.0001, steps=5 , vocab_file = None):
        """
        ...
        for i in range(steps):
            if self.sg:
                train_document_dbow(
                    ...
                    learn_words=False, learn_hidden=False, doctag_vectors=doctag_vectors, doctag_locks=doctag_locks, vocab_file = vocab_file
                )
            elif self.dm_concat:
