...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
    ...
    #print "the word to be predicted is ",word
    #print "context is ",model.wv.index2word[context_index]
    ...
    if model.negative:
        ...
        #print "............ index2word length is .................. {0}".format(len(model.wv.index2word))
...
def train_sg_pair_docs_only(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
                  context_vectors=None, context_locks=None, compute_loss=False, is_ft=False):
    if context_vectors is None:
        if is_ft:
            context_vectors_vocab = model.wv.syn0_vocab
...
def train_cbow_pair(model, word, input_word_indices, l1, alpha, learn_vectors=True, learn_hidden=True,
    ...
    it = it + 1
    context_words = []
    for index in input_word_indices:
        context_words.append(model.wv.index2word[index])
    print "context words are "
    print context_words, 
    print "\n" 
    global it
    if it > 9: 
...
def train_cbow_pair_dis2vec(model, word, input_word_indices, l1, alpha, learn_vectors=True, learn_hidden=True,
                    compute_loss=False, context_vectors=None, context_locks=None, is_ft=False):
     #print "works fine"
    ...
    if model.negative:
       ...
       # print "word is ",model.wv.index2word[word.index]
       # print "context is ",context_words
        #sys.exit()
        
        if model.wv.index2word[word.index] in model.vocabulary.dis_vocab and any(n in model.vocabulary.dis_vocab for n in context_words):
            #print "word was ",model.wv.index2word[word.index]
            #print "context is ",context_words
            #sys.exit()
            word_indices = [word.index]
            while len(word_indices) < model.negative + 1:
            ...
            if learn_hidden:
                model.syn1neg[word_indices] += outer(gb, l1)  # learn hidden -> output
            neu1e += dot(gb, l2b)  # save error
        elif model.wv.index2word[word.index] not in model.vocabulary.dis_vocab and any(n in model.vocabulary.dis_vocab for n in context_words) == False :
            # use this word (label = 1) + `negative` other random words not
            # from the cumulative distribution of disease vocabulary words (label = 0)
            word_indices = [word.index]
            while len(word_indices) < model.negative + 1:
               ...
               # print " ................... Sanity check ....................... ", model.vocabulary.cum_table_dis
                w = model.vocabulary.cum_table_dis.searchsorted(model.random.randint(model.vocabulary.cum_table_dis[-1]))
                if model.wv.vocab[model.vocabulary.index2disword[w]].index != word.index:
                    word_indices.append(model.wv.vocab[model.vocabulary.index2disword[w]].index)
            l2b = model.syn1neg[word_indices]  # 2d matrix, k+1 x layer1_size
            fb = 1. / (1. + exp(-dot(l1, l2b.T)))  # propagate hidden -> output
            gb = (model.neg_labels - fb) * alpha  # vector of error gradients multiplied by the learning rate
...
class Word2Vec(BaseWordEmbeddingsModel):
    ...
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5,
                 ...
                 trim_rule=None, sorted_vocab=1, batch_words=MAX_WORDS_IN_BATCH, compute_loss=False, vocab_file=None,test = None ,callbacks=()):
        """
        ...
        self.vocabulary = Word2VecVocab(
            ...
            sorted_vocab=bool(sorted_vocab), null_word=null_word , dis_vocab = vocab_file)
...
class Word2VecVocab(utils.SaveLoad):
    def __init__(self, max_vocab_size=None, min_count=5, sample=1e-3, sorted_vocab=True, null_word=0 , vocab_file =None , smoothing = 0.75 ):
        ...
        self.raw_vocab = None
        self.isdis = []
        self.cum_table_dis = None  # for vocabulary based negative sampling
        self.cum_table_nondis = None  # for vocabulary based negative sampling
        self.smoothing = smoothing

       


       # print "...................got a vocab file..................... {0}".format(vocab_file)
        self.dis_vocab = vocab_file
    ...
    def make_cum_table(self,wv,power=0.75, domain=2 ** 31 - 1):
        ...
        for word_ind in xrange(vocab_size):
            if wv.index2word[word_ind] in self.dis_vocab:
                self.isdis.append(0)
                self.index2disword.append(wv.index2word[word_ind])
            else:
                self.isdis.append(1)
                self.index2nondisword.append(wv.index2word[word_ind])
        disvocab_size = len(self.index2disword)
        nondisvocab_size = len(self.index2nondisword)
        self.dis_index = empty(disvocab_size, dtype=int)
        self.nondis_index = empty(nondisvocab_size, dtype=int)
        for word_ind in xrange(disvocab_size):
            self.dis_index[word_ind] = wv.vocab[self.index2disword[word_ind]].index
        for word_ind in xrange(nondisvocab_size):
            self.nondis_index[word_ind] = wv.vocab[self.index2nondisword[word_ind]].index
        self.cum_table = zeros(vocab_size, dtype=uint32)
        self.cum_table_nondis = zeros(nondisvocab_size, dtype=uint32)
        self.cum_table_dis = zeros(disvocab_size, dtype=uint32)
        # compute sum of all power (Z in paper)
        train_words_pow_dis = float(sum([wv.vocab[word].count ** self.smoothing for word in self.index2disword]))
        cumulative_dis = 0.0
        for word_index in range(disvocab_size):
            cumulative_dis += wv.vocab[self.index2disword[word_index]].count ** self.smoothing / train_words_pow_dis
            self.cum_table_dis[word_index] = round(cumulative_dis * domain)
        if len(self.cum_table_dis) > 0:
            assert self.cum_table_dis[-1] == domain
        train_words_pow_nondis = float(sum([wv.vocab[word].count ** self.smoothing for word in self.index2nondisword]))
        cumulative_nondis = 0.0
        for word_index in range(nondisvocab_size):
            cumulative_nondis += wv.vocab[self.index2nondisword[word_index]].count ** self.smoothing / train_words_pow_nondis
            self.cum_table_nondis[word_index] = round(cumulative_nondis * domain)
        if len(self.cum_table_nondis) > 0:
            assert self.cum_table_nondis[-1] == domain
        train_words_pow = float(sum([wv.vocab[word].count ** self.smoothing for word in wv.index2word]))
        cumulative = 0.0
        for word_index in range(vocab_size):
            cumulative += wv.vocab[wv.index2word[word_index]].count ** self.smoothing / train_words_pow
