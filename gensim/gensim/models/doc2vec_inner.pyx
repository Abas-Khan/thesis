...
def train_document_dbow_docs(model, doc_words, doctag_indexes, alpha, work=None,
                        train_words=False, learn_doctags=True, learn_words=True, learn_hidden=True,
                        word_vectors=None, word_locks=None, doctag_vectors=None, doctag_locks=None):
    cdef int hs = model.hs
    cdef int negative = model.negative
        ...
        for i in range(document_len):
            if _train_words:  # simultaneous skip-gram wordvec-training
                ...
                for j in range(j, k):
                    ...
                    if negative:
                        # we reuse the DBOW function, as it is equivalent to skip-gram for this purpose
                        next_random = fast_document_dbow_neg_docs(negative, cum_table, cum_table_len, _word_vectors, syn1neg, size,
                                                             indexes[i], indexes[j], _alpha, _work, next_random,
                                                             _learn_words, _learn_hidden, _word_locks)
            ...
            for j in range(doctag_len):
                if hs:
                    fast_document_dbow_hs(points[i], codes[i], codelens[i], _doctag_vectors, syn1, size, _doctag_indexes[j],
                                          _alpha, _work, _learn_doctags, _learn_hidden, _doctag_locks)
                if negative:
                    next_random = fast_document_dbow_neg_docs(negative, cum_table, cum_table_len, _doctag_vectors, syn1neg, size,
                                                             indexes[i], _doctag_indexes[j], _alpha, _work, next_random,
                                                             _learn_doctags, _learn_hidden, _doctag_locks)

    return result

cdef unsigned long long fast_document_dbow_neg_docs(
    const int negative, np.uint32_t *cum_table, unsigned long long cum_table_len,
    REAL_t *context_vectors, REAL_t *syn1neg, const int size, const np.uint32_t word_index,
    const np.uint32_t context_index, const REAL_t alpha, REAL_t *work,
    unsigned long long next_random, int learn_context, int learn_hidden, REAL_t *context_locks) nogil:
...
def train_document_dbow(model, doc_words, doctag_indexes, alpha, work=None,
    ...
    cdef np.uint32_t word_nature[MAX_DOCUMENT_LEN]
            ...
            if _train_words:  # simultaneous skip-gram wordvec-training
                ...
                for j in range(j, k):
                    ...
                    if negative:
                        ...
                        # we reuse the DBOW function, as it is equivalent to skip-gram for this purpose
                        next_random = fast_document_dbow_neg(negative, cum_table, cum_table_len,cum_table_dis, cum_table_dis_len, 
                                                       cum_table_nondis, cum_table_nondis_len, dis_indexes, nondis_indexes, _doctag_vectors, syn1neg, size,
                                                             indexes[i], _doctag_indexes[j], word_nature[i], word_nature[j], _alpha, _work, next_random,
                                                             _learn_doctags, _learn_hidden, _doctag_locks ,max_min_optim, sample_select_dist, sampling_param, objective_param)
