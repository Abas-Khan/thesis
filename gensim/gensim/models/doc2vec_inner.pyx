...
cdef unsigned long long fast_document_dbow_neg_mod(
...
def train_document_dbow(model, doc_words, doctag_indexes, alpha, work=None,
    ...
    cdef long corpus_count_words = 0
    cdef long corpus_count_docs = 1
    ...
    for token in doc_words:
        ...
        word_nature[i] = isdis_identify[predict_word.index]
        if hs:
            ...
            if _train_words:  # simultaneous skip-gram wordvec-training
                ...
                for j in range(j, k):
                    ...
                    if negative:
                        ...
                        # we reuse the DBOW function, as it is equivalent to skip-gram for this purpose
                        next_random = fast_document_dbow_neg_mod(negative, cum_table, cum_table_len, cum_table_dis, cum_table_dis_len, 
                                                       cum_table_nondis, cum_table_nondis_len, dis_indexes, nondis_indexes, 
                                                       _word_vectors, syn1neg, size, indexes[i], indexes[j], word_nature[i], word_nature[j], 
                                                       _alpha,_work, next_random,_learn_words, _learn_hidden,_word_locks, max_min_optim, sample_select_dist, sampling_param, objective_param)
                        '''
